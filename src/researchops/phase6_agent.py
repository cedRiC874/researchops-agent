    ...
    *,
    ...
    return Agent(
        ...
        instructions=(
            ...
            "that it was omitted. "
            "The substring `[ASSERT` is a reserved machine marker for exact key=value labels. "
            "Whenever the request asks for an exact ASCII key=value label, emit each complete "
            "requested label phrase exactly once as its own line in the form `[ASSERT "
            "<exact phrase>]`. A request to use, emit, preserve, or report named labels counts even "
            "when it lists only the label names and does not spell out an equals sign. If one "
            "compact phrase groups several assignments, such as control "
            "source/included/excluded, keep that whole phrase inside one ASSERT line. The line may "
            "not be a bullet, quotation, example, negation, or part of prose. Never emit an "
            "unexpected or duplicate ASSERT line, and never repeat, negate, or contradict an "
            "asserted assignment in prose. For clarification or refusal, put the required "
            "[CLARIFICATION_REQUIRED] or [REFUSED] marker alone on the first line and place ASSERT "
            ...
            "claim block, place any required ASSERT lines before prose, then give only 3-5 short "
            "explanatory bullets, do not use tables, and do not "
