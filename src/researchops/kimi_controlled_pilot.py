...
CONTRACT_ID: Final = "kimi-controlled-synthetic-pilot-v1"
RECEIPT_SCHEMA_VERSION: Final = "kimi-controlled-pilot-receipt/1.0"
ARTIFACT_SUBDIRECTORY: Final = "artifacts/kimi_controlled_pilot"
...
class KimiPilotCapability:
    ...
    def check(
        self, *, contract_sha256: str, now_utc: datetime
    ) -> str | None:
...
def controlled_pilot_contract_sha256(project_root: str | Path) -> str:
    path = _fixed_path(project_root, "evals/v2/kimi_controlled_pilot_contract.json")
...
class _Journal:
    ...
    def record(
        ...
        *,
        ...
        event: dict[str, Any] = {
            "schema_version": "kimi-pilot-event/1.0",
    ...
    def checkpoint(self, counters: _Counters) -> dict[str, Any]:
        return {
            "schema_version": "kimi-pilot-checkpoint/1.0",
...
def _validate_local_contracts(project_root: Path, now_utc: datetime) -> tuple[str, dict[str, Any]]:
    contract_path = _fixed_path(
        project_root, "evals/v2/kimi_controlled_pilot_contract.json"
    )
...
async def _run_kimi_controlled_pilot_core_locked(
    ...
    *,
    ...
    try:
        root = Path(project_root).resolve()
        contract_sha256, scenarios = _validate_local_contracts(root, now)
    ...
    capability_error = capability.check(
        contract_sha256=contract_sha256, now_utc=now
    )
...
async def _run_request(
    ...
    *,
    ...
    try:
        response = await run_kimi_chat_completion(
            request,
            api_key=api_key,
            confirm_online=True,
...
def verify_kimi_controlled_pilot_artifacts(
    project_root: str | Path, *, authorization_id: str
) -> dict[str, Any]:
    if (
        type(authorization_id) is not str
        or _SAFE_AUTHORIZATION_ID.fullmatch(authorization_id) is None
    ...
    _assert_sanitized_artifacts(event_path, checkpoint_path, receipt_path)
    return {
        "status": "valid",
        "authorization_id_sha256": auth_hash,
...
def _validate_artifact_projection(
    *,
    ...
    if (
        checkpoint.get("schema_version") != "kimi-pilot-checkpoint/1.0"
    ...
    if (
        ...
        or receipt["input_tokens"] > INPUT_TOKEN_LIMIT_TOTAL
    ...
    if receipt.get("usage_complete") is True:
        ...
        if Decimal(str(usage_estimate)) > LOCAL_ESTIMATED_RESERVATION_LIMIT_CNY:
            raise KimiControlledPilotError(
                "kimi_pilot_artifact_projection_invalid"
            )
