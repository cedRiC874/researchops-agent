...
from .first_live_start import _ClaimedFirstLiveStart, _ClaimedFirstLiveStartV5, FirstLiveStartError
...
class _FirstLiveTimedSession(_PendingTimedLedgerSession):
    def __init__(self, owner, session, **kwargs):
        if type(owner) not in (_FirstLiveModelFactory, _FirstLiveModelFactoryV5):
...
class _FirstLiveModelFactory:
    ...
    def __init__(self, prepared, ledger):
        if type(self) not in (_FirstLiveModelFactory, _FirstLiveModelFactoryV5):
            raise FirstLiveStartError('first_live_factory_preparation_required', claim_consumed=True)
        expected_type = _ClaimedFirstLiveStart if type(self) is _FirstLiveModelFactory else _ClaimedFirstLiveStartV5
        if type(prepared) is not expected_type or prepared._environment is None or type(ledger) is not AuditLedger:
            raise FirstLiveStartError("first_live_factory_preparation_required", claim_consumed=True)
    ...
    def _check_source_and_claim(self):
        self._prepared._assert_process()
        expected = self._prepared.source_identity
        reader = verify_current_timed_profile
        if type(self) is _FirstLiveModelFactoryV5:
            from researchops_external_closure.execution_current_v4 import verify_current_timed_profile as reader
            if (expected.source_recipe_version != 4 or expected.profile != 'first_live'
                or raw(self._plan) != raw(decode_strict_json_object(self._prepared.plan_bytes, max_bytes=131072))
                or self._plan['binding']['execution_commit'] != expected.execution_commit
                or self._plan['binding']['execution_tree'] != expected.execution_tree
                or self._plan['binding']['source_integrity_commitment_sha256'] != expected.source_integrity_commitment_sha256):
                self._stop('first_live_request_source_mismatch')
        current = reader(ROOT, profile='first_live')
...
class _FirstLiveModelFactoryV5(_FirstLiveModelFactory):
    """Exact v5 owner, same fixed scenarios/caps and v1.2 terminal format."""
    pass
