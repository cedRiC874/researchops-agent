...
class TimedFirstLiveEvidenceIdentity:
    historical: TimedFirstLiveHistoricalIdentity | first_live_identity_v5.TimedFirstLiveHistoricalIdentity
...
def verify_timed_first_live_evidence(project_root, *, artifact_directory, bundle_bytes,
    ...
    """Legacy v4/v9 identity with unchanged artifact and admission semantics."""
    return _verify_timed_first_live_evidence(project_root, implementation_version=4,
        artifact_directory=artifact_directory, bundle_bytes=bundle_bytes,
        ...
        verification_time_utc=verification_time_utc, reviewed_evidence_commit=reviewed_evidence_commit,
        reviewed_evidence_tree=reviewed_evidence_tree, sensitive_canaries=sensitive_canaries)


def verify_timed_first_live_evidence_v5(project_root, *, artifact_directory, bundle_bytes,
        expected_bundle_commitment_sha256, expected_authorization_binding_sha256,
        verification_time_utc, reviewed_evidence_commit, reviewed_evidence_tree, sensitive_canaries=()):
    """Bind the same timed artifact format to explicit v5/v11 historical identity.

    No caller-supplied verifier, identity result, or claimed subject is accepted.
    A verified archive remains neither an external review nor runtime authority.
    """
    return _verify_timed_first_live_evidence(project_root, implementation_version=5,
        artifact_directory=artifact_directory, bundle_bytes=bundle_bytes,
        expected_bundle_commitment_sha256=expected_bundle_commitment_sha256,
        expected_authorization_binding_sha256=expected_authorization_binding_sha256,
        verification_time_utc=verification_time_utc, reviewed_evidence_commit=reviewed_evidence_commit,
...
def _verify_timed_first_live_evidence(project_root, *, implementation_version, artifact_directory, bundle_bytes,
        expected_bundle_commitment_sha256, expected_authorization_binding_sha256,
        verification_time_utc, reviewed_evidence_commit, reviewed_evidence_tree, sensitive_canaries):
    ...
    if type(implementation_version) is not int or implementation_version not in (4, 5):
        core.fail('timed_first_live_evidence_version_invalid')
    ...
    if implementation_version == 4:
        ...
        historical = first_live_identity_v5.verify_historical_timed_implementation(
            project_root, commit=binding["execution_commit"], tree=binding["execution_tree"])
