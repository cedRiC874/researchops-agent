...
from .campaign_admission import verify_campaign_prerequisites, verify_campaign_prerequisites_v5, VerifiedCampaignPrerequisitesV5
...
class _ClaimedCampaignStartV5(_ClaimedCampaignStart):
    ...
    def _assert_process(self):
        ...
        if type(self.plan_bytes) is not bytes or digest(self.plan_bytes) != self._plan_bytes_sha256:
            self._taken = True; self._clock.abort_new_work()
            raise CampaignStartError('campaign_start_plan_changed', claim_consumed=True)
...
def _source_functions(version):
    if type(version) is not int or version not in (4, 5):
        raise ValueError('campaign_start_version_invalid')
    if version == 4:
        return PROFILE_PATHS, verify_campaign_prerequisites, verify_local_timed_execution_identity, verify_current_timed_profile
    from researchops_external_closure import execution_components_v4, execution_local_v4, execution_current_v4
    return (execution_components_v4.PROFILE_PATHS, verify_campaign_prerequisites_v5,
...
def _prepare_process_bound_campaign_start(root, documents, *, external_observation_bundle, timing_plan_bytes, admission_inputs):
    return _prepare_process_bound_campaign_start_impl(root, documents, external_observation_bundle=external_observation_bundle,
        timing_plan_bytes=timing_plan_bytes, admission_inputs=admission_inputs, implementation_version=4)


def _prepare_process_bound_campaign_start_v5(root, documents, *, external_observation_bundle, timing_plan_bytes, admission_inputs):
    return _prepare_process_bound_campaign_start_impl(root, documents, external_observation_bundle=external_observation_bundle,
        timing_plan_bytes=timing_plan_bytes, admission_inputs=admission_inputs, implementation_version=5)


def _prepare_process_bound_campaign_start_impl(root, documents, *, external_observation_bundle, timing_plan_bytes, admission_inputs, implementation_version):
    consumed = False; winner = clock = None
    try:
        paths, verify_prerequisites, verify_local, verify_current = _source_functions(implementation_version)
        ...
        source_plan = decode_strict_json_object(_current_file(root, paths['campaign'][1]), max_bytes=2_000_000)
        ...
        proof = verify_prerequisites(root, documents, external_observation_bundle=external_observation_bundle,
            timing_plan_bytes=timing_plan_bytes, admission_inputs=admission_inputs, verification_time_utc=validity.started_at)
        if implementation_version == 5 and type(proof) is not VerifiedCampaignPrerequisitesV5:
            raise ValueError('campaign_start_prerequisites_invalid')
        ...
        identity = verify_local(root, profile='campaign', expected_commit=execution['execution_commit'],
        ...
        current = verify_current(root, profile='campaign')
        ...
        environment = verify_process_environment(root, **expected_environment)
        _stamp, deadline = validity.checkpoint(reserve_ns=sealing); clock._tighten_phase_deadline(deadline)
        result_type = _ClaimedCampaignStart if implementation_version == 4 else _ClaimedCampaignStartV5
        return result_type(_TOKEN, clock=clock, validity=validity, proof=proof, identity=identity, environment=environment,
