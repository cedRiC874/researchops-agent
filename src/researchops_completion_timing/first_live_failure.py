...
from .first_live_start import FirstLiveStartError, _ClaimedFirstLiveStart, _ClaimedFirstLiveStartV5
...
def build_first_live_failure_receipt(root, error, *, factory=None):
    ...
    if safe is None:
        safe = ("first_live_publication_failed" if type(error) is FirstLivePublishError else
                ...
                "first_live_runtime_failed" if type(factory) in (_FirstLiveModelFactory, _FirstLiveModelFactoryV5) else "first_live_failure_unknown")
    ...
    elif type(factory) in (_FirstLiveModelFactory, _FirstLiveModelFactoryV5):
        try:
            prepared = vars(factory)["_prepared"]
            expected = _ClaimedFirstLiveStart if type(factory) is _FirstLiveModelFactory else _ClaimedFirstLiveStartV5
            if type(prepared) is not expected:
