...
class _CampaignTimedSession(_PendingTimedLedgerSession):
    def __init__(self, owner, session, **kwargs):
        if type(owner) not in (_CampaignModelFactory, _CampaignModelFactoryV5): raise TypeError('campaign owner required')
...
class _CampaignModelFactory:
    def __init__(self, task_owner, ledger):
        if type(self) not in (_CampaignModelFactory, _CampaignModelFactoryV5):
            raise CampaignRuntimeError('campaign_runtime_owner_required')
        expected = _CampaignTaskOwner if type(self) is _CampaignModelFactory else _CampaignTaskOwnerV5
        if type(task_owner) is not expected or type(ledger) is not AuditLedger:
            raise CampaignRuntimeError('campaign_runtime_owner_required')
    ...
    async def run_case(self, case_index, *, api_key):
        ...
        try:
            ...
            async with DeepSeekProvider().open_model(model_id=self._execution['model_id'], api_key=api_key,
                ...
                stamp = self._ledger._now()
                self._sdk_started_at = stamp.removesuffix('+00:00') + 'Z' if stamp.endswith('+00:00') else stamp
...
class _CampaignModelFactoryV5(_CampaignModelFactory):
    """Same execution semantics, bound to the exact v5 task owner."""
    pass
