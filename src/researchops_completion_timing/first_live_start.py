...
class _StartBudget:
    def __init__(self):
        ...
        # Pair UTC with the earlier monotonic sample before input/source checks.
        # Expiry is authenticated later, but preparation time must not disappear
        # if UTC stalls while those checks are running.
        self._start_utc = self.last_utc = parse_utc_timestamp(_utc_now())
        self.deadline_ns = self.start_ns + _WHOLE_NS
        self._expiry_deadline_ns = None
    ...
    def checkpoint(self, expires_at_utc=None, *, reserve_ns=_SEAL_NS):
        ...
        if expires_at_utc is not None:
            expiry = parse_utc_timestamp(expires_at_utc)
            remaining = control._duration(utc, expiry) * 1_000_000_000
            if remaining <= 0:
                raise ValueError("first_live_start_authorization_expired")
            initial = control._duration(self._start_utc, expiry) * 1_000_000_000
            translated = min(self.start_ns + initial.numerator // initial.denominator,
                             now_ns + remaining.numerator // remaining.denominator)
            self._expiry_deadline_ns = (translated if self._expiry_deadline_ns is None
                                       else min(self._expiry_deadline_ns, translated))
        if self._expiry_deadline_ns is not None:
            if now_ns >= self._expiry_deadline_ns:
                raise ValueError("first_live_start_authorization_expired")
            deadline = min(deadline, self._expiry_deadline_ns)
...
class _ClaimedFirstLiveStartV5(_ClaimedFirstLiveStart):
    """Separate exact type; the legacy Provider factory still rejects this."""
    __slots__ = ('_plan_bytes_sha256',)

    def __init__(self, token, **values):
        super().__init__(token, **values)
        self._plan_bytes_sha256 = digest(self.plan_bytes)

    def _assert_process(self):
        super()._assert_process()
        if type(self.plan_bytes) is not bytes or digest(self.plan_bytes) != self._plan_bytes_sha256:
            self._taken = True
            self._clock.abort_new_work()
            raise FirstLiveStartError('first_live_start_plan_changed', claim_consumed=True, origin='preparation')

    def abort(self):
        # Cleanup remains available after plan tampering, without re-validating
        # the corrupted plan or making the consumed owner reusable.
        super()._assert_process()
        with self._lock:
            self._taken = True
            self._clock.abort_new_work()
    ...
    def summary(self):
        return dict(super().summary(), implementation_version=5, source_recipe_version=4)


def _source_functions(implementation_version):
    if type(implementation_version) is not int or implementation_version not in (4, 5):
        raise ValueError('first_live_start_version_invalid')
    if implementation_version == 4:
        return PROFILE_PATHS, load_timed_implementation_contract, verify_local_timed_execution_identity, verify_current_timed_profile
    from . import first_live_identity_v5
    from researchops_external_closure import execution_components_v4, execution_local_v4, execution_current_v4
    return (execution_components_v4.PROFILE_PATHS, first_live_identity_v5.load_timed_implementation_contract,
            execution_local_v4.verify_local_timed_execution_identity, execution_current_v4.verify_current_timed_profile)


def _prepare_claimed_first_live_start_impl(root, *, plan_bytes, authorization_bytes, expected_authorization_binding_sha256,
                                         _budget, _implementation_version=4):
    """Private preparation only. No public dispatcher or Provider accepts this yet."""
    consumed = False
    clock = None
    winner = None
    try:
        paths, load_protocol, verify_local, verify_current = _source_functions(_implementation_version)
        ...
        source_plan = decode_strict_json_object(_current_file(root, paths["first_live"][1]), max_bytes=2_000_000)
        identity = verify_local(root, profile="first_live", expected_commit=proof.execution_commit,
        ...
        current = verify_current(root, profile="first_live")
        ...
        clock._tighten_phase_deadline(phase_deadline)
        result_type = _ClaimedFirstLiveStart if _implementation_version == 4 else _ClaimedFirstLiveStartV5
        return result_type(_TOKEN, clock=clock, clock_domain_id=domain, receipt_bytes=receipt,
...
def _prepare_claimed_first_live_start_v5(root, *, plan_bytes, authorization_bytes, expected_authorization_binding_sha256):
    return _prepare_claimed_first_live_start_impl(root, plan_bytes=plan_bytes, authorization_bytes=authorization_bytes,
        expected_authorization_binding_sha256=expected_authorization_binding_sha256, _budget=None, _implementation_version=5)
...
def _prepare_process_bound_first_live_start_v5(root, *, plan_bytes, authorization_bytes, expected_authorization_binding_sha256):
    return _prepare_process_bound_first_live_start_impl(root, plan_bytes=plan_bytes, authorization_bytes=authorization_bytes,
        expected_authorization_binding_sha256=expected_authorization_binding_sha256, _budget=_StartBudget(), _implementation_version=5)


def _prepare_process_bound_first_live_start_impl(root, *, plan_bytes, authorization_bytes, expected_authorization_binding_sha256,
                                               _budget, _implementation_version=4):
    """Require this loaded package's worktree; still no Provider/task/Key release."""
    prepared = None
    try:
        paths, _load_protocol, _verify_local, _verify_current = _source_functions(_implementation_version)
