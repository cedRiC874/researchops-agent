...
from .first_live_runtime import ROOT, _FirstLiveModelFactory, _FirstLiveModelFactoryV5
...
def _publish_completed_first_live(factory, directory, *, authorization_bytes, sensitive_canaries=()):
    return _publish_completed_first_live_impl(factory, directory, authorization_bytes=authorization_bytes,
        sensitive_canaries=sensitive_canaries, implementation_version=4)


def _publish_completed_first_live_v5(factory, directory, *, authorization_bytes, sensitive_canaries=()):
    return _publish_completed_first_live_impl(factory, directory, authorization_bytes=authorization_bytes,
        sensitive_canaries=sensitive_canaries, implementation_version=5)


def _publish_completed_first_live_impl(factory, directory, *, authorization_bytes, sensitive_canaries, implementation_version):
    ...
    try:
        ...
        expected = _FirstLiveModelFactory if implementation_version == 4 else _FirstLiveModelFactoryV5
        if type(factory) is not expected or not isinstance(directory, Path):
            raise ValueError("first_live_publish_input_invalid")
        ...
        started = prepared.clock._sealing_offset() if implementation_version == 5 else None
        if implementation_version == 5: factory._check_source_and_claim()
        profile, _ = archive.load_first_live_artifact_contract(ROOT)
        if started is None: started = prepared.clock._sealing_offset()
        ...
        if implementation_version == 5:
            factory._check_source_and_claim()
            checkpoint()
        ...
        if type(factory) in (_FirstLiveModelFactory, _FirstLiveModelFactoryV5):
