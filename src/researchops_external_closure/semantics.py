...
_TIMED_TERMINAL_VERSION = 'provider-completion-ledger-event/1.2'
_MAX_TIMED_TERMINAL_PAYLOAD_BYTES = 262144
_TIMING_CONTRACT_SHA256 = '7490576433693a45520e82bb161ee371d5a3087fa65975f5a1ac7418f6b13c6f'
_LOCAL_CLAIM_CONTRACT_SHA256 = '10e6a6f50712b9f6b7bdf1bcf628f1745d2bfcf5723fd9f7d230ea682ee90695'
_TIMED_REFERENCE_FIELDS = frozenset({'timing_contract_sha256', 'plan_commitment_sha256',
    'execution_binding_sha256', 'clock_domain_id', 'segment_commitment_sha256'})
...
def _validate_plan(
    ...
    *,
    ...
    fields = _EXTERNAL_RUNTIME_PLAN_FIELDS | ({'timing_plan_commitment_sha256', 'execution_scope'} if timed else set())
    _exact(plan, frozenset(fields), "closure_denominator_invalid")
    if timed:
        scope = _exact(plan['execution_scope'], frozenset({'mode', 'execution_environment_id',
            'local_claim_contract_sha256', 'cross_host_execution_allowed', 'temporary_store_override_allowed'}),
            'closure_denominator_invalid')
        if (not _is_sha256(plan['timing_plan_commitment_sha256'])
                or scope['mode'] != 'single_host_fixed_store'
                or type(scope['execution_environment_id']) is not str
                or re.fullmatch(r'PCEENV-[A-F0-9]{32}', scope['execution_environment_id']) is None
                or scope['local_claim_contract_sha256'] != _LOCAL_CLAIM_CONTRACT_SHA256
                or scope['cross_host_execution_allowed'] is not False
                or scope['temporary_store_override_allowed'] is not False):
...
def _validate_event_skeleton(
    ...
    *, terminal_timing_references: Mapping[str, Any] | None = None,
) -> None:
    ...
    for events in decoded_event_runs:
        ...
        for event in events:
            ...
            if terminal_timing_references is not None and event_type in _TERMINAL_EVENT_TYPES:
                if len(event.row['safe_payload_json'].encode('utf-8')) > _MAX_TIMED_TERMINAL_PAYLOAD_BYTES:
                    _fail('closure_event_projection_invalid')
    ...
    *,
    ...
    for ordinal, (run, events) in enumerate(zip(ordered_runs, decoded)):
        ...
        while events[cursor].row["event_type"] == "model_request_started":
            ...
            expected_terminal_base = dict(expected_start)
            if terminal_timing_references is not None:
                if str(attempt_index) not in terminal_timing_references:
                    _fail('closure_event_projection_invalid')
                expected_terminal_base.update(schema_version=_TIMED_TERMINAL_VERSION,
                    timing=terminal_timing_references[str(attempt_index)])
            if (
                ...
                or any(not _json_equal(terminal.payload.get(field), value)
                       for field, value in expected_terminal_base.items())
            ):
                _fail("closure_event_projection_invalid")
...
def _expected_terminal_payload(
    ...
    *,
    ...
    if terminal_timing_references is not None:
        if str(attempt.attempt_index) not in terminal_timing_references:
            _fail('closure_event_projection_invalid')
        payload.update(schema_version=_TIMED_TERMINAL_VERSION,
                       timing=terminal_timing_references[str(attempt.attempt_index)])
    ...
    for ordinal, (run, events) in enumerate(
        ...
        while cursor < len(events) and events[cursor].row["event_type"] == "model_request_started":
            ...
            expected_event_type, expected_terminal = _expected_terminal_payload(
                ...
                terminal_timing_references=terminal_timing_references,
            )
            if (
...
def _validate_outer_telemetry(
    ...
    *,
    ...
    if timed:
        # The producer intentionally makes no derived closure assertion. Accept
        # only its exact false diagnostic marker, then return independently
        # recomputed reasons; never use that marker as a closure permission.
        marker = {'claim_allowed': False, 'claim_scope': 'producer_payloads_only',
                  'reasons': ['independent_outer_verification_required']}
        if type(closure) is not dict or not _json_equal(closure, marker):
            _fail('closure_outer_projection_invalid')
...
def _verify_artifact_content_semantics(
    telemetry: Mapping[str, Any],
    audit_index: Mapping[str, Any],
    *,
    ...
    plan = _validate_plan(
        ...
        timed=terminal_timing_references is not None,
    )
    runs = _snapshot_rows(
    ...
    if terminal_timing_references is not None:
        # Internal timed composition only. These references must later be tied
        # to full segment/plan/publication validation by the timed evaluator.
        # Matching references alone never proves valid durations or timing.
        terminal_timing_references = _snapshot_mapping(terminal_timing_references, 'closure_event_projection_invalid')
        terminal_indices = [event.payload.get('attempt_index') for events_in_run in decoded
                            for event in events_in_run if event.row['event_type'] in _TERMINAL_EVENT_TYPES]
        if (any(not _safe_int(index, maximum=_MAX_ATTEMPTS - 1) for index in terminal_indices)
                or len(set(terminal_indices)) != len(terminal_indices)
                or set(terminal_timing_references) != {str(index) for index in terminal_indices}):
            _fail('closure_event_projection_invalid')
        for reference in terminal_timing_references.values():
            _exact(reference, _TIMED_REFERENCE_FIELDS, 'closure_event_projection_invalid')
            if (reference['timing_contract_sha256'] != _TIMING_CONTRACT_SHA256
                    or any(not _is_sha256(reference[name]) for name in
                           ('plan_commitment_sha256', 'execution_binding_sha256', 'segment_commitment_sha256'))
                    or type(reference['clock_domain_id']) is not str
                    or re.fullmatch(r'PCECLOCK-[A-F0-9]{32}', reference['clock_domain_id']) is None):
                _fail('closure_event_projection_invalid')
    _validate_event_skeleton(decoded, terminal_timing_references=terminal_timing_references)
...
def verify_artifact_content_semantics(
    ...
    *,
    ...
    """Legacy v1.1 entry; no timed-reference or custom-verifier argument.

    Timed callers require a separate full timing composition, not a rewritten
    legacy event stream. This wrapper deliberately cannot opt into v1.2.
    """
    return _verify_artifact_content_semantics(**locals())
