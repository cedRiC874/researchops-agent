"""Write the non-executable Depth-60 source-integrity successor v4 once.

The generator never overwrites v1, v2, v3, or an existing v4 plan. It performs
no Provider, model, or other network call.
"""
...
def main() -> int:
    ...
    if target.exists():
        print(
            ...
            f"{DEPTH60_SUCCESSOR_V4_PLAN_PATH.as_posix()}",
            file=sys.stderr,
        )
        return 2
    ...
    plan = build_depth60_successor_plan_v4(ROOT, locked_at_utc=locked_at)
    try:
    ...
    validation = validate_phase6_depth60_plan(ROOT, DEPTH60_SUCCESSOR_V4_PLAN_PATH)
    payload = target.read_bytes()
    print(
        json.dumps(
            {
                "successor_plan": DEPTH60_SUCCESSOR_V4_PLAN_PATH.as_posix(),
