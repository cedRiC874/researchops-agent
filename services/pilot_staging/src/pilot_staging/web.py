...
<body>
  <main class="shell">
    ...
    <section id="complete" class="card" hidden>
      <h2>已完成</h2>
      <p><strong>你已经完成全部任务，现在可以直接关闭此页面；无需点击任何按钮。</strong></p>
      <p>感谢参与。你的反馈将只按去标识化聚合口径报告。</p>
      <details class="withdrawal-panel">
        <summary>需要撤回参与或请求删除参与者级数据？</summary>
        <p class="hint">这不是普通的“结束”操作。撤回后，已提交反馈和普通运行 telemetry 不会计入可用性指标，参与者级数据将在最多 7 天内处理删除；必要的去标识安全事件和最小审计记录可能继续保留并影响安全门禁。</p>
        <button id="withdraw-complete" class="link-button" type="button">撤回参与并排除已提交反馈</button>
      </details>
    </section>
    <footer><button id="withdraw" class="link-button" type="button" hidden>撤回参与并请求删除本次数据</button></footer>
  </main>
...
const consentItems={
 ...
 withdrawal_understood:'我知道可以随时撤回参与；撤回会排除可用性反馈和普通运行 telemetry，并阻止后续模型调用，但必要的去标识安全事件和最小审计记录可能保留。',
...
async function loadState(){
  ...
  if(data.status==='complete'){show('login',false);show('consent',false);show('task',false);show('complete');show('withdraw',false);return;}
