...
Current status is `preflight_implemented_offline_tested_not_run`. The dedicated Models preflight CLI
and MockTransport fixtures are implemented, but no authenticated metadata request has run. There is no
Chat Completions adapter, runtime registration, Chat request, tool call, token observation, cost
evidence, pilot result or model-quality evidence. The machine-readable boundary is
[`evals/v2/kimi_provider_contract.json`](../evals/v2/kimi_provider_contract.json).

This design reviewed public first-party documentation only. It did not read a Key or send an
authenticated request to `api.moonshot.cn`. Existing DeepSeek, Anthropic, candidate, pilot and private
...
request with zero model-generation calls, zero client retries and no response-body persistence. It is
covered only by offline fixtures and defaults to `not_run / network_calls=0`; no live request exists.
...
Even a successful receipt could establish only China-platform authentication and exact `kimi-k3`
visibility at one point in time. It would not verify or authorize Chat Completions, tool calling,
...
1. Keep the implemented fixed-origin Models preflight fixture-only until candidate v5 and its
   successor pilot pack pass clean offline CI; no Chat Completions adapter is enabled.
2. Separately authorize at most one zero-generation-token metadata request. Success remains
   non-authorizing; failure stops and requires review before any new authorization.
