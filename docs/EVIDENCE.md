...
| Kimi 中国区 Models preflight / candidate v5（本地 successor） | 固定 `GET /v1/models` 已实现并只用 MockTransport 离线测试；commitment `105b7def…5165dffc` predecessor 为 v4，不继承历史结果；Chat/通用在线入口关闭，未 live preflight、未注册、未调用模型；synthetic-only，private拒绝 | [Kimi 边界](KIMI_PROVIDER.md)、[provider contract](../evals/v2/kimi_provider_contract.json)、[preflight contract](../evals/v2/kimi_models_preflight_contract.json)、[candidate v5](../evals/v2/public_regression_candidate_v5.json) |
...
| Pilot staging（历史 main CI） | `main@3dfb0a36` pilot run 32932614805 通过 51 项 offline contracts、1 项真实 PostgreSQL contract 与无 Provider secret Compose；该 run 早于本地 candidate v5/supervised v6，不验证本地 successor | [main pilot run](https://github.com/cedRiC874/researchops-agent/actions/runs/32932614805)、[服务验证](../services/pilot_staging/VERIFICATION.md)、[PR #19 main 快照](evidence/anthropic-models-preflight-main-ci-v1/README.md) |
| Pilot staging candidate v5/v6（本地未提交 successor） | candidate v5 verifier 与 v6 pack 绑定仅完成本地无网络验证；v6 仍使用 DeepSeek，Kimi live preflight/model calls 均为 false，未继承历史结果，也不能引用上述 main run 作为 v5/v6 clean-CI 证据 | [服务验证](../services/pilot_staging/VERIFICATION.md)、[candidate v5](../evals/v2/public_regression_candidate_v5.json)、[v6 pack](../services/pilot_staging/content/pilot_pack.supervised_v6.json)、[v6 review](../services/pilot_staging/content/pilot_pack.supervised_v6.review.json) |
