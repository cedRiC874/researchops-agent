...
| Eval v2 private custodian kit v1.1 | 8 个严格 schema、不同 Ed25519 keys、调用方提供的外部 anchors、两阶段 ledger、denominator/budget/suppression 算术通过 synthetic offline gate；真实 private release 固定拒绝，当前 request/access/claims 均为 false | [kit](../evals/v2/private_holdout_kit/README.md)、[guide](PRIVATE_HOLDOUT_CUSTODIAN_KIT.md)、[protocol](../evals/v2/private_holdout_kit/protocol.json) |
