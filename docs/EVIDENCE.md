...
| DeepSeek Depth-60 interpretation audit v1 | Post-hoc only；机械 40 题 first-gate 为 ASSERT inventory 20 / numeric prose 12 / plain literal 3 / assignment prose 2 / enum prose 2 / malformed ASSERT 1，但 required-phrases 40 实际分为 content-bearing 38 与 byte-empty 2；delivery-aware cross-tab 为 30/4/3/1，DEV-043/060 两条零字节 delivery failure 另列，普通 literal omission 实为 1；ASSERT deficit、五题 matcher defect 与两条空交付是至少三种非穷尽、非互斥 findings；39/40 与 scorer-input hash 字节一致，P6-DEV-057 仅可做脱敏投影；反例绑定 Python 3.12.13 patch 与 Unicode 15.0.0；`ancova`/`itt-boundary` 严格 0/4 但各 3 题仅格式失败；54/60 仅暴露 score concentration，不是 meaningful accuracy | [审计说明](evidence/phase6-deepseek-depth60-interpretation-audit-v1/README.md)、[拒绝原因图](evidence/phase6-deepseek-depth60-interpretation-audit-v1/rejection_reason_histogram.svg)、[机器拒绝投影](evidence/phase6-deepseek-depth60-interpretation-audit-v1/rejection_reason_histogram.json)、[machine projection](evidence/phase6-deepseek-depth60-interpretation-audit-v1/diagnostic_projection.json)、[95% lexical counterexample](evidence/phase6-deepseek-depth60-interpretation-audit-v1/95_percent_lexical_counterexample.json) |
...
| Kimi Candidate v8 / Chat-Pilot v3 diagnostic successor（未在线运行且不可执行） | commitment `b41269ac…1f9e962c`；v3保持v2 request bytes、接受谓词和优先级，仅为`kimi_chat_response_invalid`增加39项固定本地branch code；诊断子对象仅含schema+code，不含raw header/body、Provider ID/hash、字段值、offset、size、prompt/reasoning/tool payload、授权binding或free text。不继承v7失败或授权；public/Pilot online authorization固定为false；Kimi仍未注册、Provider 1/2 | [Candidate v8](../evals/v2/public_regression_candidate_v8.json)、[Chat v3 contract](../evals/v2/kimi_chat_completions_contract_v3.json)、[Pilot v3 contract](../evals/v2/kimi_controlled_pilot_contract_v3.json)、[v3 runbook](KIMI_CONTROLLED_PILOT_V3_RUNBOOK.md) |
