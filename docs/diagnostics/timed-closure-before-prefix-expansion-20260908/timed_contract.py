...
CONTRACT_SHA256='19681888a0b7b686f36e9a6503b6c82efd406c63ad5509e7ea519b1984281265'
CONTRACT_BYTES=18841
...
def validate_document(kind,payload,*,root=ROOT):
    ...
    if kind in {'receipt','ledger'}:
        ...
        if kind=='receipt':
            if value['artifact_publication_disposition']=='normal' and value['bundle_status']!='complete':
                partial=value['partial_artifacts']; count=len(partial)
                completed=contract['timed_write_stages'].index(value['timing']['last_completed_write_stage'])
                names=contract['artifact_write_order'][:count]
                if set(partial)!=set(names) or count not in {completed,min(completed+1,7)}:
                    _fail('partial_prefix_invalid')
                for index,name in enumerate(names):
                    if partial[name]['bytes']==0 and (index<completed or partial[name]['sha256']!=_sha(b'')):
                        _fail('partial_empty_file_invalid')
