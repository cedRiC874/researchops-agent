...
async def run_timed_campaign(*, documents, external_observation_bundle, timing_plan_bytes, admission_inputs,
    ...
    try:
        ...
        from .campaign_start import _prepare_process_bound_campaign_start
        from .campaign_tasks import _CampaignTaskOwner
        from .campaign_runtime import _CampaignModelFactory
        from .campaign_phase import _run_owned_campaign_phase
        from .campaign_workspace import _owned_campaign_work_paths
        from .campaign_publish import _publish_completed_campaign, _publication_checkpoint
        from .first_live_publish import _write_exclusive
        stage='admission'
        prepared=_prepare_process_bound_campaign_start(ROOT,documents,external_observation_bundle=external_observation_bundle,
        ...
        stage='task_opening'; owner=_CampaignTaskOwner(prepared); owner.open_tasks(task_package_path)
        stage='workspace'
        with _owned_campaign_work_paths(ROOT,identifier) as (database,directory,summary_path):
            closed=False
            factory=_CampaignModelFactory(owner,AuditLedger(database))
            stage='phase'
            ...
            publication=_publish_completed_campaign(factory,directory)
...
def main(argv=None):
    ...
    if '--confirm-online' not in args:
        ...
        try:
            ...
            result=asyncio.run(run_timed_campaign(**arguments,confirm_online=parsed.confirm_online))
