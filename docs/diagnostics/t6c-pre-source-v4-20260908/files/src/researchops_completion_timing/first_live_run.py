...
from .first_live_publish import _publish_completed_first_live, _write_exclusive, FirstLivePublishError
...
async def _run_with_budget(*, plan_bytes, authorization_bytes, expected_authorization_binding_sha256, budget):
    ...
    try:
        ...
        with _owned_work_paths(ROOT, identifier) as (database, directory, envelope_path):
            try:
                factory = _FirstLiveModelFactory(prepared, AuditLedger(database))
                ...
                publication = _publish_completed_first_live(factory, directory, authorization_bytes=authorization_bytes)
...
def main(argv=None):
    parser = _Parser(description="Run one explicitly authorized timed first-live validation.")
    ...
    try:
        ...
        if not args.confirm_online:
            ...
            result = asyncio.run(_run_with_budget(plan_bytes=plan, authorization_bytes=auth,
                expected_authorization_binding_sha256=args.expected_authorization_binding, budget=budget))
