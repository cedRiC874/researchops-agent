...
"""Frozen diagnostic successor orchestrator for Kimi candidate v8.

This module is an additive fork of the immutable v2 orchestrator.  It binds
the acceptance-equivalent parser v3, a new contract/artifact/receipt namespace
and a runtime-supplied candidate-v8 commitment.  It deliberately does not
inherit either consumed predecessor authorization or post-lock result.
"""
...
from .kimi_chat_transport_v3 import (
    KIMI_CHAT_MODEL_ID,
...
CONTRACT_ID: Final = "kimi-controlled-synthetic-pilot-v3"
RECEIPT_SCHEMA_VERSION: Final = "kimi-controlled-pilot-receipt/3.0"
EVENT_SCHEMA_VERSION: Final = "kimi-pilot-event/3.0"
CHECKPOINT_SCHEMA_VERSION: Final = "kimi-pilot-checkpoint/3.0"
AUTHORIZATION_SCHEMA_VERSION: Final = "kimi-pilot-authorization/3.0"
SUCCESSOR_TOMBSTONE_SCHEMA_VERSION: Final = (
    "kimi-pilot-successor-tombstone/3.0"
)
SUCCESSOR_TOMBSTONE_FILENAME: Final = "successor_v3_tombstone.json"
ARTIFACT_SUBDIRECTORY: Final = "artifacts/kimi_controlled_pilot_v3"
LEGACY_ARTIFACT_SUBDIRECTORY: Final = "artifacts/kimi_controlled_pilot"
PREDECESSOR_ARTIFACT_SUBDIRECTORY: Final = "artifacts/kimi_controlled_pilot_v2"
KIMI_CHAT_PROVIDER_ID: Final = "moonshot_kimi"
KIMI_CHAT_TRANSPORT_ID: Final = KIMI_CHAT_TRANSPORT_V3_ID
...
# Filled only after the independent Chat v3 contract is frozen.  This value is
...
_RESPONSE_VALIDATION_DIAGNOSTIC_FIELDS: Final = frozenset(
    {"schema_version", "code"}
)


def _validated_response_validation_diagnostic(
    value: object,
    *,
    error_code: str | None,
) -> dict[str, str] | None:
    if error_code != "kimi_chat_response_invalid":
        if value is not None:
            raise KimiControlledPilotError(
                "kimi_pilot_response_diagnostic_invalid"
            )
        return None
    if (
        not isinstance(value, dict)
        or set(value) != _RESPONSE_VALIDATION_DIAGNOSTIC_FIELDS
        or value.get("schema_version")
        != RESPONSE_VALIDATION_DIAGNOSTIC_SCHEMA_VERSION
        or value.get("code") not in RESPONSE_VALIDATION_DIAGNOSTIC_CODES
    ):
        raise KimiControlledPilotError(
            "kimi_pilot_response_diagnostic_invalid"
        )
...
def kimi_controlled_pilot_contract() -> dict[str, Any]:
    return {
        ...
        "successor_binding": {
            ...
            "authoritative_v8_manifest_required_by_run_and_verifier": True,
            "authoritative_v8_manifest_path": (
        ...
        "artifacts": {
            ...
            "predecessor_subdirectory_checked_before_key_load": (
                PREDECESSOR_ARTIFACT_SUBDIRECTORY
            ),
            ...
            "legacy_successor_tombstone_sha256_bound_to_all_v3_artifacts": True,
        ...
        "response_validation_diagnostic": {
            ...
            "same_error_projection": [
                "request_failed",
                "run_terminal",
                "checkpoint",
                "receipt",
            ],
            "cross_projection_equality_condition": (
                "terminal_error_remains_kimi_chat_response_invalid"
            ),
            "cross_projection_equality_required_when_condition_met": True,
            "terminal_guard_error_override_clears_terminal_projection": True,
            "request_failed_diagnostic_remains_hash_bound_after_override": True,
            "fixed_local_branch_only": True,
            "causal_provider_fault_claim_allowed": False,
            "raw_header_body_identifier_field_value_offset_size_allowed": False,
        ...
        "evaluation_boundary": {
            "synthetic_only": True,
            "production_entrypoint_online_authorized": False,
            "production_entrypoint_stable_error_code": (
                "kimi_pilot_v8_online_not_authorized"
            ),
            "test_seam_mock_transport_only": True,
...
def controlled_pilot_contract_sha256(project_root: str | Path) -> str:
    path = _fixed_path(
        project_root, "evals/v2/kimi_controlled_pilot_contract_v3.json"
    )
...
class _Journal:
    def __init__(
        ...
        *,
        ...
        self.response_validation_diagnostic: dict[str, str] | None = None
    ...
    def record(
        ...
        *,
        ...
        response_validation_diagnostic: Mapping[str, Any] | None = None,
    ) -> None:
        self.state = state
        self.error_code = error_code
        if response_validation_diagnostic is not None:
            diagnostic_projection = _validated_response_validation_diagnostic(
                dict(response_validation_diagnostic), error_code=error_code
            )
            self.response_validation_diagnostic = diagnostic_projection
        elif error_code == "kimi_chat_response_invalid":
            diagnostic_projection = _validated_response_validation_diagnostic(
                self.response_validation_diagnostic, error_code=error_code
            )
        else:
            diagnostic_projection = None
            self.response_validation_diagnostic = None
        self.outcome_unknown = outcome_unknown
        ...
        event: dict[str, Any] = {
            ...
            "response_validation_diagnostic": diagnostic_projection,
            "prev_hash": prev_hash,
        }
    ...
    def checkpoint(self, counters: _Counters) -> dict[str, Any]:
        return {
            ...
            "response_validation_diagnostic": (
                dict(self.response_validation_diagnostic)
                if self.response_validation_diagnostic is not None
                else None
            ),
...
def _validate_authoritative_candidate(
    ...
    *,
    ...
    candidate_path = _fixed_path(
        project_root, "evals/v2/public_regression_candidate_v8.json"
    )
    try:
        if candidate_validator is None:
            # Lazy import avoids the eval_v2_freeze -> pilot_v3 import cycle.
...
def _validate_local_contracts(
    ...
    *,
    ...
    chat_contract_path = _fixed_path(
        project_root, "evals/v2/kimi_chat_completions_contract_v3.json"
    )
...
def _receipt(
    *,
    ...
    diagnostic_projection = _validated_response_validation_diagnostic(
        (
            dict(response_validation_diagnostic)
            if response_validation_diagnostic is not None
            else None
        ),
        error_code=error_code,
...
async def run_kimi_controlled_pilot(
    ...
    *,
    ...
    """Production entry point remains disabled for the diagnostic-only v8 snapshot."""

    del project_root, confirm_online, _key_loader
...
async def _run_kimi_controlled_pilot_core_locked(
    ...
    *,
    ...
    predecessor_artifact_root = _fixed_path(
        root, PREDECESSOR_ARTIFACT_SUBDIRECTORY
    )
    run_directory = artifact_root / auth_hash
    legacy_run_directory = legacy_artifact_root / auth_hash
    predecessor_run_directory = predecessor_artifact_root / auth_hash
    ...
    if (
        legacy_run_directory.exists()
        or legacy_run_directory.is_symlink()
        or predecessor_run_directory.exists()
        or predecessor_run_directory.is_symlink()
    ):
        return _not_run_receipt(
            now_utc=now,
...
async def _run_request(
    ...
    *,
    ...
    try:
        response = await run_kimi_chat_completion_v3(
            request,
            api_key=api_key,
            confirm_online=True,
        ...
        diagnostic = response_validation_diagnostic(exc)
        try:
...
def _finalize_failure(
    ...
    *,
    ...
    receipt = _receipt(
        ...
        chain_head=journal.events[-1]["event_hash"],
        response_validation_diagnostic=(
            journal.response_validation_diagnostic
        ),
    )
    journal.write_receipt(receipt)
    return receipt
...
def _validate_event_state_machine(
    ...
    *,
    ...
    for index, event in enumerate(events):
        try:
            diagnostic_projection = _validated_response_validation_diagnostic(
                event.get("response_validation_diagnostic"),
                error_code=event.get("error_code"),
            )
        except KimiControlledPilotError:
            _fsm_invalid()
        if diagnostic_projection != event.get("response_validation_diagnostic"):
            _fsm_invalid()
        if terminal_seen:
            _fsm_invalid()
        ...
        if event_type == "run_terminal":
            ...
            if (
                ...
                and event.get("error_code")
                == failure_marker.get("error_code")
                and event.get("response_validation_diagnostic")
                != failure_marker.get("response_validation_diagnostic")
            ):
                _fsm_invalid()
...
KimiPilotV3Capability = KimiPilotCapability
build_kimi_pricing_attestation_v3 = build_kimi_pricing_attestation
build_kimi_terms_attestation_v3 = build_kimi_terms_attestation
controlled_pilot_v3_contract_sha256 = controlled_pilot_contract_sha256
create_kimi_pilot_v3_capability = create_kimi_pilot_capability
kimi_controlled_pilot_v3_contract = kimi_controlled_pilot_contract
load_kimi_pricing_attestation_v3 = load_kimi_pricing_attestation
load_kimi_terms_attestation_v3 = load_kimi_terms_attestation
run_kimi_controlled_pilot_v3 = run_kimi_controlled_pilot
...
__all__ = [
    ...
    "build_kimi_pricing_attestation_v3",
    "build_kimi_terms_attestation",
    "build_kimi_terms_attestation_v3",
    "controlled_pilot_contract_sha256",
    "controlled_pilot_v3_contract_sha256",
    "create_kimi_pilot_capability",
    "create_kimi_pilot_v3_capability",
    "kimi_controlled_pilot_contract",
    "kimi_controlled_pilot_v3_contract",
    "load_kimi_pricing_attestation",
    "load_kimi_pricing_attestation_v3",
    "load_kimi_terms_attestation",
    "load_kimi_terms_attestation_v3",
    "run_kimi_controlled_pilot",
    "run_kimi_controlled_pilot_v3",
    "verify_kimi_controlled_pilot_artifacts",
    "verify_kimi_controlled_pilot_v3_artifacts",
]
