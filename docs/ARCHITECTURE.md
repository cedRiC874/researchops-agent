...
| 模式 | 被测对象 | 当前证据 | 不能声称什么 |
| --- | --- | --- | --- |
| `offline_deterministic` | 数据质量、方法选择、统计工具、报告、重试、审批和审计控制面 | `main@c65ff65c` run 32957003253 为 50/50、21/21、profile valid；旧 44/50 事故保留审计 | 真实 LLM 的规划成功率，或脱离 source/data/manifest 复用成绩 |
...
| Kimi 中国区 Models preflight / candidate v5 | 固定 `api.moonshot.cn` GET、Bearer credential 隔离、exact `kimi-k3`、一次 attempt、零 retry/fallback/redirect、64 KiB cap、strict receipt；Chat/通用在线入口关闭，public/pilot Provider 仍为 DeepSeek | PR #21 已进入 `main@c65ff65c`；candidate `105b7def…5165dffc` 是 pre-call snapshot，post-lock 一次 GET 为 HTTP 200、1/1 call、exact model visible、0 model tokens、cost null且不继承；[长期快照](evidence/kimi-models-preflight-main-ci-v1/README.md) | Chat/Responses/tools、usage/cost semantics、质量、non-synthetic/private 或已注册第二 Provider |
| Kimi Chat/Pilot diagnostic successor / candidate v8 | Chat v3保持v2 request bytes、接受谓词和优先级，仅为 broad response-invalid 增加39项固定本地branch code；Pilot v3在terminal仍为response-invalid时强制failure/terminal/checkpoint/receipt一致，terminal guard覆盖时仅保留hash-bound request-failure diagnostic；v6/v7 CLI永久禁用，v8任何确认状态下均0 Key读取/0 call | candidate `b41269ac…1f9e962c`、Chat/Pilot v3均为离线冻结；v7独立post-lock失败不继承，public/Pilot online authorized=false；active staging仍为Candidate v5/Pack6且未修改；[runbook](KIMI_CONTROLLED_PILOT_V3_RUNBOOK.md) | Provider因果故障、Kimi usage/tool/error兼容、实际tokens/bill/latency、质量、注册、private/non-synthetic |
