...
from researchops import kimi_controlled_pilot_v2 as predecessor_pilot_module
from researchops import kimi_controlled_pilot_v3 as pilot_module
from researchops.kimi_controlled_pilot_v3 import (
...
from researchops.kimi_controlled_pilot_v3 import run_kimi_controlled_pilot as production_run
...
CANDIDATE_COMMITMENT = hashlib.sha256(b"candidate-v8").hexdigest()
...
def _candidate_validator(**kwargs):
    ...
    if candidate_path.name != "public_regression_candidate_v8.json":
        raise AssertionError("candidate path is not fixed to v8")
    return {
        "status": "valid",
...
def _copy_contract_root(directory: str) -> Path:
    ...
    for name in (
        "kimi_controlled_pilot_contract_v3.json",
        "kimi_controlled_pilot_contract_v2.json",
        "kimi_controlled_pilot_contract.json",
        "kimi_chat_completions_contract_v3.json",
...
class KimiControlledPilotTests(unittest.TestCase):
    def test_v3_runbook_publishes_only_offline_commands(self) -> None:
        ...
        self.assertIn("no executable online command", runbook)
        for online_flag in (
            "--confirm-online",
            "--accept-successor-v8-locked-caps",
            ...
            self.assertNotIn(online_flag, runbook)

    def test_machine_contract_is_exact_and_all_claims_false(self) -> None:
        snapshot = json.loads(
            (ROOT / "evals/v2/kimi_controlled_pilot_contract_v3.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        self.assertEqual(
            snapshot["evaluation_boundary"]["production_entrypoint_stable_error_code"],
            "kimi_pilot_v8_online_not_authorized",
        )
        self.assertTrue(
            snapshot["evaluation_boundary"]["test_seam_mock_transport_only"]
        )
    ...
    def test_production_runner_is_diagnostic_only_and_never_loads_key(self) -> None:
        def fail_key_loader() -> str:
            raise AssertionError("production diagnostic snapshot must not load a Key")
        ...
        receipt = asyncio.run(
            production_run(
                ...
                confirm_online=True,
                capability=None,
                _key_loader=fail_key_loader,
            )
        )
        self.assertEqual(receipt["status"], "not_run")
        self.assertEqual(
            receipt["error_code"], "kimi_pilot_v8_online_not_authorized"
        )
        ...
        self.assertNotIn("key_loader_passed_to_runner", receipt)
        ...
        for case in cases:
            with self.subTest(case=case), tempfile.TemporaryDirectory() as directory:
                ...
                if case == "wrong_contract":
                    path = root / "evals/v2/kimi_controlled_pilot_contract_v3.json"
                    payload = json.loads(path.read_text(encoding="utf-8"))
    ...
    def test_successor_binding_is_explicit_and_v1_raw_is_immutable(self) -> None:
        ...
        self.assertEqual(snapshot["contract_id"], "kimi-controlled-synthetic-pilot-v3")
        self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(ValueError):
                build_kimi_terms_attestation(
                    authorization_id=AUTHORIZATION_ID,
                    contract_sha256=contract_sha256,
                    candidate_id=CANDIDATE_ID.replace("v8", "v7"),
                    candidate_commitment_sha256=CANDIDATE_COMMITMENT,
                    chat_contract_sha256=CHAT_CONTRACT_SHA256,
                    authorization_expires_at_utc=NOW + timedelta(hours=1),
    ...
    def test_legacy_authorization_artifact_is_rejected_before_key_load(self) -> None:
        for predecessor_namespace in (
            "kimi_controlled_pilot",
            "kimi_controlled_pilot_v2",
        ):
            with self.subTest(namespace=predecessor_namespace), tempfile.TemporaryDirectory() as directory:
                root = _copy_contract_root(directory)
                auth_hash = hashlib.sha256(AUTHORIZATION_ID.encode()).hexdigest()
                (root / "artifacts" / predecessor_namespace / auth_hash).mkdir(
                    parents=True
                )
                key_reads = 0
                transport_calls = 0
    ...
    def test_v3_artifacts_are_candidate_bound_and_legacy_verifiers_reject_them(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = _copy_contract_root(directory)
            receipt = asyncio.run(
                run_kimi_controlled_pilot(
            ...
            auth_hash = hashlib.sha256(AUTHORIZATION_ID.encode()).hexdigest()
            v3_run = root / ARTIFACT_SUBDIRECTORY / auth_hash
            for artifact_name in ("event_chain.jsonl", "checkpoint.json", "receipt.json"):
                self.assertIn(
                    CANDIDATE_COMMITMENT,
                    (v3_run / artifact_name).read_text(encoding="utf-8"),
                )
            ...
            v2_contract_sha256 = (
                predecessor_pilot_module.controlled_pilot_contract_sha256(root)
            )
            v2_candidate_id = predecessor_pilot_module.EXPECTED_CANDIDATE_ID
            v2_candidate_commitment = hashlib.sha256(
                b"offline-predecessor-candidate-v7"
            ).hexdigest()
            v2_expiry = NOW + timedelta(hours=1)
            v2_terms = predecessor_pilot_module.build_kimi_terms_attestation(
                authorization_id=AUTHORIZATION_ID,
                contract_sha256=v2_contract_sha256,
                candidate_id=v2_candidate_id,
                candidate_commitment_sha256=v2_candidate_commitment,
                chat_contract_sha256=predecessor_pilot_module._CHAT_CONTRACT_SHA256,
                authorization_expires_at_utc=v2_expiry,
                retrieved_at_utc=NOW - timedelta(minutes=5),
                canonical_text_sha256=TERMS_TEXT_HASHES,
            )
            v2_pricing = predecessor_pilot_module.build_kimi_pricing_attestation(
                authorization_id=AUTHORIZATION_ID,
                contract_sha256=v2_contract_sha256,
                candidate_id=v2_candidate_id,
                candidate_commitment_sha256=v2_candidate_commitment,
                chat_contract_sha256=predecessor_pilot_module._CHAT_CONTRACT_SHA256,
                authorization_expires_at_utc=v2_expiry,
                terms_attestation_sha256=v2_terms.attestation_sha256,
                retrieved_at_utc=NOW - timedelta(minutes=5),
                canonical_source_sha256=PRICING_SOURCE_HASH,
                canonical_source_bytes=PRICING_SOURCE_BYTES,
            )
            v2_capability = predecessor_pilot_module.create_kimi_pilot_capability(
                authorization_id=AUTHORIZATION_ID,
                contract_sha256=v2_contract_sha256,
                candidate_id=v2_candidate_id,
                candidate_commitment_sha256=v2_candidate_commitment,
                chat_contract_sha256=predecessor_pilot_module._CHAT_CONTRACT_SHA256,
                expires_at_utc=v2_expiry,
                terms_attestation=v2_terms,
                pricing_attestation=v2_pricing,
            )
            v2_key_reads = 0

            def v2_key_loader() -> str:
                nonlocal v2_key_reads
                v2_key_reads += 1
                return KEY_CANARY

            def v2_candidate_validator(**kwargs):
                self.assertEqual(
                    Path(kwargs["candidate_path"]).name,
                ...
                return {
                    ...
                    "candidate_commitment_sha256": v2_candidate_commitment,
                    "predecessor_candidate_commitment_sha256": (
                        predecessor_pilot_module.PREDECESSOR_CANDIDATE_COMMITMENT_SHA256
                    ),
                    "prior_results_inherited": False,
                    "predecessor_failure_result_inherited": False,
                    "predecessor_authorization_reused": False,
            ...
            v2_result = asyncio.run(
                predecessor_pilot_module._run_kimi_controlled_pilot_for_test(
                    root,
                    confirm_online=True,
                    capability=v2_capability,
                    _key_loader=v2_key_loader,
                    _transport_factory=lambda: self.fail(
                        "v2 runner must stop at the v3 successor tombstone"
                    ),
                    _now=lambda: NOW,
                    _candidate_validator=v2_candidate_validator,
                )
            )
            self.assertEqual(v2_result["status"], "not_run")
            self.assertEqual(
                v2_result["error_code"],
                "kimi_pilot_authorization_reused_across_versions",
            )
            self.assertEqual(v2_key_reads, 0)
            ...
            with self.subTest(status=status), tempfile.TemporaryDirectory() as directory:
                ...
                receipt = asyncio.run(
                    run_kimi_controlled_pilot(
                        ...
                        confirm_online=True,
                        capability=_capability(
                            root, f"AUTH-KIMI-V3-HTTP-{status}"
                        ),
                        _key_loader=lambda: KEY_CANARY,
                        _transport_factory=lambda: httpx.MockTransport(handler),
                        _now=lambda: NOW,
            ...
            with self.subTest(stream=name), tempfile.TemporaryDirectory() as directory:
                ...
                receipt = asyncio.run(
                    run_kimi_controlled_pilot(
                        ...
                        confirm_online=True,
                        capability=_capability(
                            root, f"AUTH-KIMI-V3-STREAM-{name.upper()}"
                        ),
                        _key_loader=lambda: KEY_CANARY,
                        _transport_factory=lambda: httpx.MockTransport(handler),
                        _now=lambda: NOW,
    ...
    def test_response_diagnostic_is_structurally_consistent_and_sanitized(self) -> None:
        expected = {
            "schema_version": "kimi-response-validation-diagnostic/1.0",
            "code": "top_level_field_set_invalid",
        }

        def run_failure(root: Path, authorization_id: str) -> Path:
            payload = _chunk(1, delta={"role": "assistant"})
            payload["extension"] = "RAW-DIAGNOSTIC-CANARY"

            def handler(request: httpx.Request) -> httpx.Response:
                return _sse_response(request, _event(payload))
            ...
            self.assertEqual(receipt["status"], "failed")
            self.assertEqual(receipt["error_code"], "kimi_chat_response_invalid")
            self.assertEqual(receipt["response_validation_diagnostic"], expected)
            auth_hash = hashlib.sha256(authorization_id.encode()).hexdigest()
            run_directory = root / ARTIFACT_SUBDIRECTORY / auth_hash
            events = [
                json.loads(line)
            ...
            self.assertEqual(
                [event["response_validation_diagnostic"] for event in events],
                [None, None, expected, expected],
            )
            checkpoint = json.loads(
                (run_directory / "checkpoint.json").read_text(encoding="utf-8")
            )
            self.assertEqual(checkpoint["response_validation_diagnostic"], expected)
            for name in ("event_chain.jsonl", "checkpoint.json", "receipt.json"):
                text = (run_directory / name).read_text(encoding="utf-8")
                self.assertNotIn("RAW-DIAGNOSTIC-CANARY", text)
                self.assertNotIn("extension", text)
                self.assertNotIn(KEY_CANARY, text)
            self.assertEqual(
                verify_kimi_controlled_pilot_artifacts(
                    root, authorization_id=authorization_id
                )["status"],
        ...
        for mutation in ("missing", "unknown", "drift"):
            with self.subTest(mutation=mutation), tempfile.TemporaryDirectory() as directory:
                root = _copy_contract_root(directory)
                authorization_id = f"AUTH-KIMI-V3-DIAG-{mutation.upper()}"
                run_directory = run_failure(root, authorization_id)
                events = [
                    json.loads(line)
                    for line in (run_directory / "event_chain.jsonl")
                    .read_text(encoding="utf-8")
                ...
                if mutation == "missing":
                    changed = None
                    for event in events:
                        event["response_validation_diagnostic"] = None
                elif mutation == "unknown":
                    changed = {
                        "schema_version": "kimi-response-validation-diagnostic/1.0",
                        "code": "provider_free_text",
                    }
                    for event in events[-2:]:
                        event["response_validation_diagnostic"] = changed
                else:
                    changed = {
                        "schema_version": "kimi-response-validation-diagnostic/1.0",
                        "code": "media_type_not_event_stream",
                    }
                    events[-1]["response_validation_diagnostic"] = changed
                for name in ("checkpoint.json", "receipt.json"):
                    path = run_directory / name
                    payload = json.loads(path.read_text(encoding="utf-8"))
                    payload["response_validation_diagnostic"] = changed
                    path.write_text(
                        json.dumps(payload, sort_keys=True, separators=(",", ":"))
                        + "\n",
                        encoding="utf-8",
    ...
    def test_terminal_guard_override_keeps_request_diagnostic_only(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = _copy_contract_root(directory)
            authorization_id = "AUTH-KIMI-V3-DIAG-EXPIRY"
            response_returned = False
            payload = _chunk(1, delta={"role": "assistant"})
            payload["extension"] = "not-persisted"
            ...
            receipt = asyncio.run(
                run_kimi_controlled_pilot(
                    ...
                    capability=_capability(
                        root,
                        authorization_id,
                        expires_at_utc=NOW + timedelta(seconds=600),
                    ),
                    _key_loader=lambda: KEY_CANARY,
                    _transport_factory=lambda: httpx.MockTransport(handler),
                    _now=clock,
            ...
            self.assertEqual(receipt["error_code"], "kimi_pilot_capability_expired")
            self.assertIsNone(receipt["response_validation_diagnostic"])
            auth_hash = hashlib.sha256(authorization_id.encode()).hexdigest()
            events = [
                json.loads(line)
                for line in (
                    root / ARTIFACT_SUBDIRECTORY / auth_hash / "event_chain.jsonl"
                )
                .read_text(encoding="utf-8")
                .splitlines()
            ]
            self.assertEqual(
                events[-2]["response_validation_diagnostic"],
                {
                    "schema_version": "kimi-response-validation-diagnostic/1.0",
                    "code": "top_level_field_set_invalid",
                },
            )
            self.assertIsNone(events[-1]["response_validation_diagnostic"])
