...
from researchops import kimi_chat_transport_v2 as transport_v2
...
_KEY = "offline-kimi-v2-key-CANARY-0123456789"
_REQUEST_ID = "request-offline-v2-001"
_COMPLETION_ID = "cmpl-offline-v2-001"
_RAW_CANARY = "RAW-PROVIDER-V2-CANARY"
_OMIT = object()
...
class KimiChatTransportV2Tests(unittest.TestCase):
    def test_versioned_api_is_distinct_and_reuses_only_frozen_controls(self) -> None:
        self.assertEqual(KIMI_CHAT_PARSER_VERSION, "2.0")
        self.assertEqual(
            KIMI_CHAT_TRANSPORT_V2_ID,
            "moonshot_direct_chat_completions_sse_v2",
        )
        self.assertNotEqual(KIMI_CHAT_TRANSPORT_V2_ID, v1.KIMI_CHAT_TRANSPORT_ID)
        self.assertIsNot(
            transport_v2._parse_success_response_v2,
            v1._parse_success_response,
        )
        self.assertIn(
            "terminal usage projection allowlist", transport_v2.__doc__ or ""
        )
    ...
    def test_official_source_commitments_are_exact_and_immutable(self) -> None:
        ...
        with self.assertRaises(AttributeError):
            KIMI_CHAT_V2_OFFICIAL_SOURCE_COMMITMENTS[0].sha256 = "0" * 64  # type: ignore[misc]
    ...
    def test_id_model_index_created_and_role_are_exact_and_stable(self) -> None:
        ...
        changed_id = _stop_stream().replace(
            _COMPLETION_ID.encode(), b"cmpl-offline-v2-002", 1
        )
        with self.assertRaises(KimiChatTransportError):
    ...
    def test_confirmation_and_configuration_fail_before_key_loader(self) -> None:
        ...
        with self.assertRaises(KimiChatTransportError) as confirmation:
            asyncio.run(
                run_kimi_chat_completion_v2(_request(), _key_loader=loader)
            )
        self.assertEqual(confirmation.exception.code, "kimi_chat_confirmation_required")
        self.assertEqual(loaded, 0)

        with patch.object(transport_v2, "KIMI_CHAT_MODEL_ID", "drift"):
